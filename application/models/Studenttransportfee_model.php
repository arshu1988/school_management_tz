...
class Studenttransportfee_model extends CI_Model {
    ...
    public function getTransportFeeByMonth($student_data) {
        $this->db->select()->from('student_transport_fees');
        $this->db->where(array('student_session_id' => $student_data['student_session_id'], 'Month' => $student_data['Month'], 'Year' =>$student_data['Year']));
        $this->db->order_by('id');
        $query = $this->db->get();
        return $query->result_array(); 
    }
