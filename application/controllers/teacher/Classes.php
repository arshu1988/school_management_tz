...
class classes extends CI_Controller {
    ...
    function create() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/class/classCreate', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $data = array(
                'class' => $this->input->post('class'),
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->class_model->add($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">Class update successfully</div>');
            redirect('teacher/classes'); 
        }
        $class_result = $this->class_model->get();
        $data['classlist'] = $class_result;
    ...
    function addsection($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/teacher/header', $data);
            $this->load->view('teacher/class/sectionAdd', $data);
            $this->load->view('layout/teacher/footer', $data);
        } else {
            $data = array(
                'class_id' => $id,
            ...
            redirect('teacher/classes/addsection/' . $id);
        }
    }
