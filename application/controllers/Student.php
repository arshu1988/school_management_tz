...
class student extends Admin_Controller {
    ...
    function view($id) {
        ...
        $data['subjectslist'] = $this->class_model->getSubjectByClass($student['class_id'], $student['section_id']);
        ...
        if (!empty($examList)) {
            ...
            foreach ($examList as $ex_key => $ex_value) {
                ...
                $array['exam_type'] = $ex_value['category_id'];
                $array['exam_label'] = $ex_value['label'];
                $array['exam_result'] = $x;
                $new_array[] = $array;
            }
            $data['examSchedule'] = $new_array;
        }
        //echo "<pre>";print_r($data['examSchedule']);die;

        //$studentAttendance = $this->stuattendence_model->searchAttendenceStudentSession($student['class_id'], //$student['section_id'],$student['id']);die('hell0');
        $data['extras'] = $this->student_model->get_exams_extra($student['id']);
       // $data['attendance']['total'] = count($studentAttendance);
        //$_count = 0;
        //foreach($studentAttendance as $_attendance){
          //  if($_attendance['attendence_type_id']==1) $_count++;
       // }
        //$data['attendance']['attended'] = $_count;
    ...
    function create() {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $transport_fees = $this->input->post('transport_fees');
...
public function doupload($id) {
    ...
    function marksheet_pdf(){
        $content = $this->input->post("content");
        $html = $this->load->view('student/studentsMarksheetPdf', array('html'=>$content), true);
        //$html = $this->load->view('student/studentsMarksheetPdf', null, true);
        //file_put_contents("uploads/mark.html", $html);
        //$html =  file_get_contents("uploads/mark.html");
        while($pos = strpos($html, '<input class="cell-input" style="display:none;width:100%;" type="text">') !== false ) {
            $html = str_replace('<input class="cell-input" style="display:none;width:100%;" type="text">','', $html);
        }
        while($pos = strpos($html, '<input class="cell-input" type="text" style="display:none;width:100%;">') !== false ) {
            $html = str_replace('<input class="cell-input" type="text" style="display:none;width:100%;">','', $html);
        }
        
        //$html = substr_replace('<input class="cell-input" type="text" style="display:none;width:100%;">','', $html);
        //die(strpos($html, '<input class="cell-input" type="text" style="display:none;width:100%;">//');
        //echo $html;die;
    ...
    function extras(){
        ...
        $remarkFor = $this->input->post('remark_for');
        $note = $this->input->post('note');
        $this->student_model->save_exams_extra($studentId,$remarkFor,$remark,$note);
    }
    function getMarksheet($id){
        //$data['title'] = 'Student Details';
        $student = $this->student_model->get($id);
        $gradeList = $this->grade_model->get();
        //$student_due_fee = $this->studentfee_model->getDueFeeBystudent($student['class_id'], $student['section_id'], $id);
        ...
        $studentAttendance = $this->stuattendence_model->searchAttendenceStudentSession($student['class_id'], $student['section_id'],$student['id']);
        $data['extras'] = $this->student_model->get_exams_extra($student['id']);
        $data['attendance']['total'] = count($studentAttendance);
        $_count = 0;
        foreach($studentAttendance as $_attendance){
        ...
        $data['gradeList'] = $gradeList;
        $data['student'] = $student;
        echo $this->load->view('student/halfMarksheet', $data,true).$this->load->view('student/fullMarksheet', $data,true);
    }
    function bulkMarksheet(){
        $marksheetType = $this->input->post('type');
        $studentIdsStr = $this->input->post('students');
        ...
        foreach ($studentIds as $index =>$id) {
            ...
            $data['gradeList'] = $gradeList;
            $data['student'] = $student;
            $data['download_button'] = false;
            if($type=="half"){
                $content .= $this->load->view('student/halfMarksheet', $data,true);
            } else{
                $content .= $this->load->view('student/fullMarksheet', $data,true);
            }
            if(count($studentIds)>$index+1)//Check if this is alst element
                    $content .= "<pagebreak />";
