    ...
    function add_AjaxTransportfee() {
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
                ...
                'transport_month' => form_error('Month'),
...
$student_data=array('student_session_id' => $this->input->post('student_session_id'), 'Month' => $this->input->post('Month'), 'Year' => $this->input->post('Year'));
$datass=$this->studenttransportfee_model->getTransportFeeByMonth($student_data);
$fees_per_month=$this->input->post('fees_per_month');

if($datass!=''){

foreach($datass as $feedata){

  $totalfees=$totalfees+$feedata['amount'];
}
...
$totalfees=0;
      ...
      if($totalfees+$this->input->post('amount') <= $fees_per_month){
            $inserted_id = $this->studenttransportfee_model->add($data);
            $array = array('status' => 'success', 'error' => '');
            echo json_encode($array);
        }else{
       $this->form_validation->set_rules('amount_more', 'amount_more', 'required|trim|xss_clean');
$this->form_validation->set_message('required', 'The Amount For this month is already Payed');
if ($this->form_validation->run() == false) {
       //$this->form_validation->set_rules('Month', 'Month', 'required|trim|xss_clean');
       $data = array(
                'transport_amount_more' => form_error('amount_more')
    ...
    function printFeesByName() {
        ...
        $html = $this->load->view('print/printFeesByName', $data,true);
