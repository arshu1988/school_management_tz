                    ...
                    <div class="nav-tabs-custom">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active table-responsive no-padding" id="tab_1">
                                    ...
                                    <tbody>
                                        ...
                                        if (empty($resultlist)) {
                                            ...
                                            foreach ($resultlist as $key => $student) {
                                                ?>
                                                <tr class="" data-studentid="<?php echo $student['id']; ?>">
                                                    ...
                                                    <td class="pull-right">
                                                        <a href="#" class="btn btn-default btn-xs marksheet-moodal" data-studentId="<?php echo $student['id']; ?>" data-nstudentId="<?php echo isset($resultlist[$key+1])?$resultlist[$key+1]['id']:''; ?>" data-pstudentId="<?php echo isset($resultlist[$key-1])?$resultlist[$key-1]['id']:''; ?>"  data-toggle="tooltip" title="Markseet" >
                                ...
                                <div>
                                    <a href="javascript:void(0);" class="btn btn-default btn-xs bulk-download-marksheet" data-type="half"><i class="fa fa-download"></i> Term Marksheets</a>
                                    <a href="javascript:void(0);" class="btn btn-default btn-xs bulk-download-marksheet" data-type="full"><i class="fa fa-download"></i> Full Marksheets</a>
                                </div>
                            </div>                           
                                                            ...
                                                            <address>
                                                                ...
                                                                <b><?php echo $this->lang->line('date_of_birth'); ?>:</b>
                                                                    ...
                                                                    <div class="col-xs-6 col-md-6">
                                                                        ...
                                                                        <b><?php echo $this->lang->line('guardian_name'); ?>:&nbsp;</b><?php echo $student['father_name'] ?><br>
...
<div id="marksheetModal" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class="modal-title">Marksheets</h4>
            </div>
            <div class="modal-body">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="#half-marksheet" data-toggle="tab" aria-expanded="true">Term-1 Marksheet</a></li>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default pull-left prev-result"><i class="fa fa-caret-left"> Prev</i></button>
                <button type="button" class="btn btn-default pull-right next-result">Next <i class="fa fa-caret-right"></i></button>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    $(document).on('click', '.marksheet-moodal', function () {
        //$('.modal-title').html("");
        var thisTr = $(this).closest('tr');
        thisTr.addClass("active");
        var student_id = $(thisTr).data('studentid');
        $("#marksheetModal").modal('show');
        fetchMarksheet(student_id);
    });
    $(document).on('click', '.next-result', function () {
        var thisTr = $('.search-results tr.active');
        if($(thisTr).next('tr').length){
            thisTr.removeClass("active");
            thisTr.next('tr').addClass("active");
            var student_id = thisTr.next('tr').data('studentid');
            fetchMarksheet(student_id);
        } else {
    ...
    $(document).on('click', '.prev-result', function () {
        var thisTr = $('.search-results tr.active');
        if($(thisTr).prev('tr').length){
            thisTr.removeClass("active");
            thisTr.prev('tr').addClass("active");
            var student_id = thisTr.prev('tr').data('studentid');
            fetchMarksheet(student_id);
        } else {
            alert("No more results");
        }
    });
    $(document).on('click','.download-marksheet', function(event){
    ...
    function fetchMarksheet(studentId){
        var base_url = '<?php echo base_url() ?>';
        $.ajax({
            type: "get",
            url: base_url + "student/getMarksheet/"+studentId,
            success: function (response) {
                $('#marksheetModal .modal-body .tab-content').html(response);
            },
            beforeSend: function(xhr){
                var loader = '<div class="text-center"><i class="fa fa-spinner fa-spin fa-3x fa-fw fa-4" aria-hidden="true" style="font-size:8em;"></i></div>';
                $('#marksheetModal .modal-body .tab-content').html(loader);
            }
        }); 
    }
    $(document).on('click',".bulk-download-marksheet",function(){
        var site_url = '<?php echo site_url("student/bulkMarksheet/") ?>';
        var selectedItems = $('input[name="student_ids[]"]:checked');
        var studentIds = new Array();
        $.each(selectedItems,function(key,item){
            studentIds.push($(item).val());
        });
        var marksheetType = $(this).data('type');
